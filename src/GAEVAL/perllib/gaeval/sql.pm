...
sub analyze{
  ...
  $self->create_GAEVAL_SUPPORT_TBL() if(!exists($argHR->{ANN_UID_TODO_LIST}));
  $self->create_GAEVAL_PROPERTIES_TBL() if(!exists($argHR->{ANN_UID_TODO_LIST}));
  
  my $uid_aref = exists($argHR->{ANN_UID_TODO_LIST})?
    $argHR->{ANN_UID_TODO_LIST}:
      exists($argHR->{ANN_UID_LIST_SQL_SELECT})?
	$self->{dbh}->selectcol_arrayref($argHR->{ANN_UID_LIST_SQL_SELECT}):
	  $self->{dbh}->selectcol_arrayref("Select uid from $self->{GAEVAL_ANN_TBL} ORDER by geneId");
  foreach my $x (@{$uid_aref}){
    ##<DEBUG>##
    print STDERR "working on $x of " . scalar(@{$uid_aref}) . "\n" if(($x > 999)&&(($x % 1000) == 0) && exists($self->{debug}) && $self->{debug});
...
sub create_GAEVAL_ANNOTATION_TBL{
  ...
  $self->{GAEVAL_ANN_TBL} = "gaeval_annotation" if(! exists($self->{GAEVAL_ANN_TBL}));
  ...
  $self->{dbh}->do("DROP TABLE IF EXISTS $self->{GAEVAL_ANN_TBL}");
  return $self->{dbh}->do("CREATE TABLE $self->{GAEVAL_ANN_TBL} $GAEVAL_ANNOTATION_CREATE_TBL");
}
...
sub create_GAEVAL_SUPPORT_TBL{
  ...
  $self->{GAEVAL_SUPPORT_TBL} = $self->{GAEVAL_ANN_TBL} . "_gaeval" if (! exists($self->{GAEVAL_SUPPORT_TBL}));
...
sub load_GAEVAL_SUPPORT_TBL{
  ...
  my $uid_aref = $self->{dbh}->selectcol_arrayref("Select uid from $self->{GAEVAL_ANN_TBL} ORDER by uid");
  foreach my $x (@{$uid_aref}){
    print STDERR "working on $x of " . scalar(@{$uid_aref}) . "\n" if(($x > 999)&&(($x % 1000) == 0) && exists($self->{debug}) && $self->{debug});
...
sub update_GaevalSupport_TBL{
  ...
  print STDERR "[sql.pm::&update_GaevalSupport_TBL] annUID=> $annUID\n" if(exists($self->{DEBUG_list_subs}) && $self->{DEBUG_list_subs});
...
sub create_GAEVAL_PROPERTIES_TBL{
  ...
  $self->{GAEVAL_PROPERTIES_TBL} = $self->{GAEVAL_ANN_TBL} . "_properties" if (! exists($self->{GAEVAL_PROPERTIES_TBL}));
...
sub update_GaevalProperties_TBL{
  ...
  print STDERR "[sql.pm::&update_GaevalProperties_TBL] annUID=> $annUID\n" if(exists($self->{DEBUG_list_subs}) && $self->{DEBUG_list_subs});
  ...
  foreach my $intSTR (keys(%{$asiINFO_hr})){
    ...
    if(exists($asiINFO_hr->{$intSTR}->{INTRONS_conflicting})){
      ...
      my $doc = (exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}))?
	((exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}->{confirmed}) && exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}->{confirmed}->{LOCAL}))||
	 (exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}->{unsupported}) && exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}->{unsupported}->{LOCAL})))?-1:1:0;
    ...
    if(exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform})){
      my $score = (exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}->{confirmed}))?1:0;
      my $doc = ((exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}->{confirmed}) && exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}->{confirmed}->{LOCAL}))||
		 (exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}->{unsupported}) && exists($asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}->{unsupported}->{LOCAL})))?-1:1;
      my $data = Data::Dumper->Dump([$asiINFO_hr->{$intSTR}->{INTRONS_alt_isoform}],['asiINFO_hr->{' . $intSTR . '}->{INTRONS_alt_isoform}']); 
      $self->{dbh}->do("$SQLins VALUES (0,$annUID,'AS_AltAnnIntron',$score,$lpos,$rpos,$doc,\"$data\")");
    }
  }
  ...
  foreach my $cmPROP ('CM_AltCPS','CM_Fission','CM_MainCPS'){
    foreach my $cps (keys(%{$cmINFO_hr->{$cmPROP}})){
      ...
      my $doc = (exists($cmINFO_hr->{$cmPROP}->{$cps}->{documented}))?exists($cmINFO_hr->{$cmPROP}->{$cps}->{documented}->{LOCAL})?-1:1:0;
  ...
  foreach my $fusdan (keys(%{$cmINFO_hr->{CM_Fusion}})){
    my $score = exists($cmINFO_hr->{CM_Fusion}->{$fusdan}->{score})?$cmINFO_hr->{CM_Fusion}->{$fusdan}->{score}:
      exists($cmINFO_hr->{CM_Fusion}->{$fusdan}->{PGS_bridged})?scalar(@{$cmINFO_hr->{CM_Fusion}->{$fusdan}->{PGS_bridged}}):
	scalar(@{$cmINFO_hr->{CM_Fusion}->{$fusdan}->{CP_bridged}});
    my $doc = exists($cmINFO_hr->{CM_Fusion}->{$fusdan}->{documented})?exists($cmINFO_hr->{CM_Fusion}->{$fusdan}->{documented}->{LOCAL})?-1:1:0;
...
sub seed_ISO_TBL{
  ...
  my $ANNselect = exists($self->{GAEVAL_ANNselect})?$self->{GAEVAL_ANNselect}:"select uid,gene_structure,gseg_gi,l_pos,r_pos FROM $self->{GAEVAL_ANN_TBL} ORDER BY gseg_gi,l_pos,r_pos";
 ...
 EVAL_ANNOTATION:
  for($ANNindx=0;$ANNindx <= $#$ANNaref;$ANNindx++){
    ...
    while(($PGSindx > 0)&&
	  (($ANNaref->[$ANNindx][2] eq $PGSaref->[$PGSindx - 1][2])&&
    ...
    while(($ANNaref->[$ANNindx][2] gt $PGSaref->[$PGSindx][2])||
	  (($ANNaref->[$ANNindx][2] eq $PGSaref->[$PGSindx][2])
	   &&($ANNaref->[$ANNindx][3] > $PGSaref->[$PGSindx][4]))){
      last EVAL_ANNOTATION if($PGSindx == $#$PGSaref);
      $PGSindx++;
    }
    
    while(($ANNaref->[$ANNindx][2] eq $PGSaref->[$PGSindx][2])&&
