...
sub loadANN{
  ...
  my $annINFO_QUERY = qq{Select * from $self->{GAEVAL_ANN_TBL} WHERE (uid = ?)};
  ...
  if(!$CDSlpos && !$CDSrpos){ ## NO CDS defined (probable pseudogene annotation)
    $self->{cds_STRUCTURE} = [];
    $self->{five_prime_UTR} = [@annSTR];
...
sub eval_annotation_support{
  ...
  print STDERR "[xgdb.pm::&eval_annotation_support] annUID = $annUID\n" if(exists($self->{DEBUG_list_subs}));
  ...
  if(exists($self->{bc_PGS}) && scalar(@{$self->{bc_PGS}})){
	##<DEBUG>##
	print STDERR "Calculating annotation support for BC ISO/PGSs (" . scalar(@{$self->{bc_PGS}}) . ")\n" if(exists($self->{debug}) && $self->{debug});
  ...
  if(exists($self->{bn_PGS}) && scalar(@{$self->{bn_PGS}})){
	##<DEBUG>##
	print STDERR "Calculating annotation support for BC+BN ISO/PGSs (" . scalar(@{$self->{bn_PGS}}) . ")\n" if(exists($self->{debug}) && $self->{debug});
  ...
  if(exists($self->{nc_PGS}) && scalar(@{$self->{nc_PGS}})){
	##<DEBUG>##
	print STDERR "Calculating annotation support for BC+NC ISO/PGSs (" . scalar(@{$self->{nc_PGS}}) . ")\n" if(exists($self->{debug}) && $self->{debug});
  ...
  if(exists($self->{nn_PGS}) && scalar(@{$self->{nn_PGS}})){
	##<DEBUG>##
	print STDERR "Calculating annotation support for BC+NN ISO/PGSs (" . scalar(@{$self->{nn_PGS}}) . ")\n" if(exists($self->{debug}) && $self->{debug});
...
sub eval_incongruence{
  my $self = shift;
  my ($annUID) = @_;

  $self->{asiINFO} = $self->_altSplicing($annUID);
  $self->{cmINFO}  = $self->_complexMRNA($annUID);
  $self->{aoaINFO} = $self->_AMBboundary($annUID);
...
sub _AMBboundary{
  ...
  my $EVIDENCE_SUPPORT_BOUNDARY_DIST = 75;
  ...
  ## Check to see if the current boundaries are already reasonably proven by BEST & COGNATE ISOs
  my $lbound = ($self->{ann_DBdata}->{strand} eq 'f')?
    ($self->{BC_MATCHES}->{BOUND_5prime} < $EVIDENCE_SUPPORT_BOUNDARY_DIST)?1:0:
    ($self->{BC_MATCHES}->{BOUND_3prime} < $EVIDENCE_SUPPORT_BOUNDARY_DIST)?1:0;
  my $rbound = ($self->{ann_DBdata}->{strand} eq 'f')?
    ($self->{BC_MATCHES}->{BOUND_3prime} < $EVIDENCE_SUPPORT_BOUNDARY_DIST)?1:0:
    ($self->{BC_MATCHES}->{BOUND_5prime} < $EVIDENCE_SUPPORT_BOUNDARY_DIST)?1:0;
  return $aoaINFO_hr if($lbound && $rbound);
  $olapWHERE .= "&&(OA.r_pos > ANN.r_pos)" if($lbound);
  $olapWHERE .= "&&(OA.l_pos < ANN.l_pos)" if($rbound);
...
sub _altSplicing{
  ...
  my $gsegID = exists($self->{ann_DBdata}->{chr})?$self->{ann_DBdata}->{chr}:$self->{ann_DBdata}->{gseg_gi};
  my $gsegID_SQL = (exists($self->{ann_DBdata}->{chr}))?"chr = $gsegID":"gseg_gi = '$gsegID'";
  ...
  foreach $ASItype (('INTRONS_alternative','INTRONS_additional','INTRONS_conflicting')){
    foreach $intSTR (keys %{$self->{BCNC_MATCHES}->{$ASItype}}){
      ...
      $resAR = $self->{dbh}->selectall_arrayref("select uid,geneId,gene_structure from $self->{GAEVAL_ANN_TBL} " .
	...
	for(my $indx=0;$indx <= $#{$self->{GAEVAL_ANN_TABLES}};$indx++){
	  next if($self->{GAEVAL_ANN_TABLES}->[$indx]->{ANN_TBL} eq $self->{GAEVAL_ANN_TBL});
	  next if($self->{GAEVAL_ANN_TABLES}->[$indx]->{dsn} eq $self->{dsn});
	  if(!exists($self->{GAEVAL_ANN_TABLES}->[$indx]->{dbh})){
	    $self->{GAEVAL_ANN_TABLES}->[$indx]->{dbh} = DBI->connect($self->{GAEVAL_ANN_TABLES}->[$indx]->{dsn,dbUSER,dbPASS},{ RaiseError => 1 });
	    $self->{GAEVAL_ANN_TABLES}->[$indx]->{dbh}->{FetchHashKeyName} = 'NAME_lc';
	  }
	  ...
	  my $SQL_IDcond = exists($self->{GAEVAL_ANN_TABLES}->[$indx]->{gsegID_conditional})?$self->{GAEVAL_ANN_TABLES}->[$indx]->{gsegID_conditional} . "'$gsegID'":$gsegID_SQL;
	  $resAR = $self->{GAEVAL_ANN_TABLES}->[$indx]->{dbh}->selectall_arrayref($SQL . " where (($SQL_IDcond)&&(l_pos <= $Ilft)&&(r_pos >= $Irgt)&&((gene_structure RLIKE '${Ilft},$Irgt' )||(gene_structure RLIKE '${Irgt},$Ilft'))) ${SQLcond}");
...
sub _complexMRNA{ ## Includes fusion (pos. polycistronic), and fission (pos. alternative C/P site) events
  ...
  if($self->group_TPS($cmINFO_hr->{CPSITES},$self->gather_TPS($annUID),$strand,$MAX_CPS_divergence,$MIN_CPS_groupsize)){
	...
	if((($self->{ann_DBdata}->{l_pos} < $cps)&&($self->{ann_DBdata}->{r_pos} > $cps))&&
	   (!exists($cmINFO_hr->{CPSITES}->{$cps}->{transcript_extensions}))&&
	   (!exists($cmINFO_hr->{CPSITES}->{$cps}->{clonepair_extensions}))&&
	   (($cps - $MAX_CPS_divergence) > $self->{BC_MATCHES}->{ISO_EXON_coverage}->[0])&&
	   (($cps + $MAX_CPS_divergence) < $self->{BC_MATCHES}->{ISO_EXON_coverage}->[$#{$self->{BC_MATCHES}->{ISO_EXON_coverage}}])){
  ...
  ## check for documentation of fusion event in annotation isoforms
  foreach my $fusdan (keys %{$cmINFO_hr->{CM_Fusion}}){
    foreach my $ANNsrc (keys %{$self->{annIsoforms}}){
      foreach my $annIso (keys %{$self->{annIsoforms}->{$ANNsrc}}){
	my $annIso_hr = $self->{annIsoforms}->{$ANNsrc}->{$annIso};
	if((($annIso_hr->{l_pos}  - 100 ) <= $cmINFO_hr->{CM_Fusion}->{$fusdan}->{lpos})&&(($annIso_hr->{r_pos} + 100) >= $cmINFO_hr->{CM_Fusion}->{$fusdan}->{rpos})){
	  $cmINFO_hr->{CM_Fusion}->{$fusdan}->{documented} = {} if(!exists($cmINFO_hr->{CM_Fusion}->{$fusdan}->{documented}));
	  $cmINFO_hr->{CM_Fusion}->{$fusdan}->{documented}->{$ANNsrc} = {} if(!exists($cmINFO_hr->{CM_Fusion}->{$fusdan}->{documented}->{$ANNsrc}));
	  $cmINFO_hr->{CM_Fusion}->{$fusdan}->{documented}->{$ANNsrc}->{$annIso} = $annIso_hr;
	}
      }
    }
  }
...
sub gather_isoforms{
  ...
  my $MIN_ISOFORM_OVERLAP_PERCENT = 0.95;

  $self->loadANN($annUID);
  return $self->{annIsoforms} if (exists($self->{annIsoforms}));
  my $gsegID = exists($self->{ann_DBdata}->{chr})?$self->{ann_DBdata}->{chr}:$self->{ann_DBdata}->{gseg_gi};
  my $gsegID_SQL = (exists($self->{ann_DBdata}->{chr}))?"chr = $gsegID":"gseg_gi = '$gsegID'"; ##JPD - need to quote variable for varchar search? 7/28/11
  ...
  my $SQL_IDcond = exists($self->{GAEVAL_ANN_TABLES}->[$indx]->{gsegID_conditional})?$self->{GAEVAL_ANN_TABLES}->[$indx]->{gsegID_conditional} . "'$gsegID'":$gsegID_SQL;
  my $resAR = $self->{dbh}->selectall_arrayref("select uid,geneId,gene_structure,l_pos,r_pos,strand from $self->{GAEVAL_ANN_TBL} where (($SQL_IDcond)&&(uid != $annUID)&&(l_pos <= $self->{ann_DBdata}->{r_pos})&&(r_pos >= $self->{ann_DBdata}->{l_pos}))");
  foreach my $annAR (@$resAR){
    ...
    foreach $in (@introns){
      ...
      my $OVERLAP = gaeval::min($self->{ann_DBdata}->{r_pos},$annAR->[4]) - gaeval::max($self->{ann_DBdata}->{l_pos},$annAR->[3]);
      if((!exists($aiINFO_hr->{LOCAL}) || !exists($aiINFO_hr->{LOCAL}->{$annAR->[0]})) && 
	 (exists($annINFO_hr->{LOCAL}->{$annUID}->{intronHASH}->{$in}) ||
	  ($OVERLAP >= $MIN_ISOFORM_OVERLAP) ||
	  (($OVERLAP / gaeval::min(($self->{ann_DBdata}->{r_pos} - $self->{ann_DBdata}->{l_pos}),($annAR->[4] - $annAR->[3]))) >= $MIN_ISOFORM_OVERLAP_PERCENT)
	 )
	){
...
## External annotation isoforms
  if(exists($self->{GAEVAL_ANN_TABLES})){
    for(my $indx=0;$indx <= $#{$self->{GAEVAL_ANN_TABLES}};$indx++){
      ...
      next if($self->{GAEVAL_ANN_TABLES}->[$indx]->{dsn} eq $self->{dsn});
      if(!exists($self->{GAEVAL_ANN_TABLES}->[$indx]->{dbh})){
	$self->{GAEVAL_ANN_TABLES}->[$indx]->{dbh} = DBI->connect($self->{GAEVAL_ANN_TABLES}->[$indx]->{'dsn'},$self->{GAEVAL_ANN_TABLES}->[$indx]->{'dbUSER'},$self->{GAEVAL_ANN_TABLES}->[$indx]->{'dbPASS'},{ RaiseError => 1 });
	$self->{GAEVAL_ANN_TABLES}->[$indx]->{dbh}->{FetchHashKeyName} = 'NAME_lc';
      }
      my $SQL = (exists($self->{GAEVAL_ANN_TABLES}->[$indx]->{ANNselect}))?$self->{GAEVAL_ANN_TABLES}->[$indx]->{ANNselect}:
	"select uid,geneId,gene_structure,l_pos,r_pos,strand from $self->{GAEVAL_ANN_TABLES}->[$INDX]->{ANN_TBL}";
      my $SQLcond = exists($self->{GAEVAL_ANN_TABLES}->[$indx]->{ANN_conditional})?$self->{GAEVAL_ANN_TABLES}->[$indx]->{ANN_conditional}:"";
      my $SQL_IDcond = exists($self->{GAEVAL_ANN_TABLES}->[$indx]->{gsegID_conditional})?$self->{GAEVAL_ANN_TABLES}->[$indx]->{gsegID_conditional} . "'$gsegID'":$gsegID_SQL;
      $resAR = $self->{GAEVAL_ANN_TABLES}->[$indx]->{dbh}->selectall_arrayref("$SQL where (($SQL_IDcond)&&(l_pos <= $self->{ann_DBdata}->{r_pos})&&(r_pos >= $self->{ann_DBdata}->{l_pos})) ${SQLcond}");
      foreach my $annAR (@$resAR){
...
sub gather_TPS{
  ...
  my ($resAR,@tps,$sqlcmd);
  for($x=0;$x<=$#{$self->{GAEVAL_ISO_TABLES}};$x++){
    $sqlcmd = "SELECT PGS.gi, PGS.pgs FROM $self->{GAEVAL_ISO_TABLES}->[$x]->{ISO_TBL} as ISO JOIN $self->{GAEVAL_ISO_TABLES}->[$x]->{PGS_TBL} as PGS ON (ISO.pgsUID = PGS.uid) JOIN  $self->{GAEVAL_ISO_TABLES}->[$x]->{SEQ_TBL} as SEQ ON (PGS.gi = SEQ.gi) WHERE(ISO.annUID = $annUID)&&(ISO.best >= 0)&&(ISO.isCognate = 1) ";
    ...
    for(my $y=0; $y<=$#$resAR;$y++){
      $resAR->[$y][1] =~ s/^\D+//;
      $resAR->[$y][1] =~ s/\D+$//;
      my @PGS = split(/\D+/,$resAR->[$y][1]);
      @PGS = sort {return $a<=>$b;} @PGS;
      push(@tps,[$resAR->[$y][0],$PGS[0],$PGS[$#PGS]]);
    }
  }
