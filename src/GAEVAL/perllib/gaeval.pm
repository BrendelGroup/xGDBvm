...
sub STRmatch{
  ...
  print STDERR "[gaeval.pm::&STRmatch]\n" if(exists($self->{DEBUG_list_subs}));
...
my $formerPOS = 0;  #temporary variable to record former annotation's start position
  for($pgsPOS=2;$pgsPOS < $#$pgsSTR_ref;$pgsPOS+=2) { #start from second one
if($pgsSTR_ref->[$formerPOS + 1]>$pgsSTR_ref->[$pgsPOS]) {
  #if the ending of former one is larger than current beginning, overlapping occurs
      my $newEnd = max($pgsSTR_ref->[$formerPOS+1], $pgsSTR_ref->[$pgsPOS+1]);
      #determine the ending position, former annotation or current one
      $pgsSTR_ref->[$formerPOS+1] = $newEnd;
      #update former annotation's ending position
      splice(@$pgsSTR_ref,$pgsPOS,2);
      #delete current start, ending position pair
      print STDERR "after splice @$pgsSTR_ref\n";
      $pgsPOS -= 2;
      #since we delete two, move anchor 2 position back
    }
      $formerPOS = $pgsPOS;
      #move to the next, and update former position with current annotation
  }
