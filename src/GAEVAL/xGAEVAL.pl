...
my $annUID_SQL = '';
my ($cleanDB,$debug,$skipAnalysis,$skipReport,$skipISEreload,$confFILE,@customEvType,@annToDo);
GetOptions("a|load_only_annotations" => sub { $argHR->{_opt_load_evidence}=0;},
	   ...
	   "load_PUT_match" => sub { $argHR->{_opt_all_evtype}=0; push(@{$argHR->{CUSTOM_EVIDENCE_TYPES}},"PUT_match");},
	   ...
	   "U|annotation_uid=i" => \@annToDo,
	   "annotation_uid_list_sql=s" => \$annUID_SQL, 
...
##### Really bad idea to hardcode this here. 
#  Use the --configuration="/DATA/Product/GAEVAL/conf/ZMGDBconf_cdnaonly.pl" on the command line instead.
#$confFILE="/DATA/Product/GAEVAL/conf/ZMGDBconf_cdnaonly.pl";

do $confFILE;# if(defined($confFILE)&&(-r $confFILE)&&($confFILE =~ /\.pl$/));

my $GA = gaeval::sql->new();
$GA->{dsn} = $argHR->{dsn};
$GA->{GAEVAL_ANN_TBL} = $argHR->{GAEVAL_ANN_TBL};
...
$GA->{GAEVAL_ANN_TABLES} = $argHR->{GAEVAL_ANN_TABLES};
$GA->{GAEVAL_ANNselect}= $argHR->{GAEVAL_ANNselect} if(exists($argHR->{GAEVAL_ANNselect}));
$GA->{debug} = $debug if($debug);
$GA->{DEBUG_list_subs} = $argHR->{DEBUG_list_subs} if exists($argHR->{DEBUG_list_subs});


$argHR->{DBhandle} = $GA->{dbh} = DBI->connect($GA->{dsn},$dbUSER,$dbPASS,{ RaiseError => 1 });
$GA->{dbh}->{FetchHashKeyName} = 'NAME_lc';
...
my $analArgHR = {};
$analArgHR->{ANN_UID_LOOP_PROCS} = \@reports if(scalar(@reports));
$analArgHR->{ANN_UID_TODO_LIST} = \@annToDo if(scalar(@annToDo));
$analArgHR->{ANN_UID_LIST_SQL_SELECT} = $annUID_SQL if($annUID_SQL ne '');
$GA->analyze($analArgHR) if(!$skipAnalysis);
