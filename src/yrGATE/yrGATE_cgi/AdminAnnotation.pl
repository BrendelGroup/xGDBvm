...
# build the AdminAnnotation table
$sql = "SELECT uid,geneId,locusId,modDate,status,USERid,chr,l_pos,r_pos,dbVer, working_group FROM user_gene_annotation WHERE status != 'SAVED' AND dbName = '$GV->{dbTitle}' ORDER BY status, modDate DESC";
...
#    my $UCAtablerow = "<td>$owner</td><td>$modDate</td><td>(&{$GV->{GenomeContextLinkFunction}}($chr,$start,$end,$dbVer))</td><td>$$ownedRef{$uid}";
  ...
  my $page = printTitle("Administrate Annotations ($GV->{dbTitle})","yrgate_admin_anno","<span class='bold indent'>Contents:</span> <span class='info'>$GV->{dbTitle} annotations: Submitted, Rejected or Approved. For ADMIN use only</span><span class='heading indent'><a href='$GV->{CGIPATH}AdminGroupView.pl'>[Aministrate Users &amp; Groups]</a></span>",1,1,'AdminAnnotation.pl');

  my $status = '';

  my $table = "<div style='clear:both; overflow:auto'><table class='mainT admin' border='1'><thead><tr class='headRow'><th>UID</th><th>Annotation ID<br /><span class='smaller_white'>(click to check out)</span></th><th>Locus ID</th><th>Owner</th><th>Working_Group</th><th>dbVer</th><th>Last Modified</th><th>Chromosome / Location</th><th>Checked out by Admin?</th></tr></thead>\n";
...
<script type='text/javascript' src='$GV->{JSPATH}popup.js'></script>
 <style type='text/css'>
   a#admin_anno {font-weight:normal; color: white; background: #08508E; padding:5px 4px 6px 4px; border: none;}
 </style>
...
<form name='tFrm' class="annotation_form" method='post' action="$GV->{CGIPATH}AdminAnnotation.pl">
...
print $page;
print "<br /><input type='submit' value='Reload Page' /><br /><input type='hidden' name='openV' value='$openV' />";
