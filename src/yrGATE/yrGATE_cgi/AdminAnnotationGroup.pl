...
if(!$PRM->{USERid}){
  ...
  bailOut("You must <a href=\"/src/yrGATE/$GV->{dbTitle}/login.pl\">log in</a> before using this tool. $GV->{dbTitle}");
}
...
# build the AdminGroupAnnotationAll table (load only records for which this user has user_group ADMIN status)
#$sql = "SELECT uid,geneId,locusId,modDate,status,USERid,chr,l_pos,r_pos,dbVer,working_group FROM user_gene_annotation WHERE status != 'SAVED' AND dbName LIKE '%GDB' AND working_group IN (SELECT private_group FROM user_group WHERE (gdb='$GV->{dbTitle}'or gdb='ALL') and user='$PRM->{USERid}' and status='ADMIN') ORDER BY status, modDate DESC";
$sql = "SELECT uid,geneId,locusId,modDate,status,USERid,chr,l_pos,r_pos,dbVer,working_group FROM user_gene_annotation WHERE status != 'SAVED' AND dbName ='$GV->{dbTitle}' AND working_group IN (SELECT private_group FROM yrgate.user_group WHERE (gdb='$GV->{dbTitle}'or gdb='ALL') and user='$PRM->{USERid}' and status='ADMIN') ORDER BY status, modDate DESC";

  $UCAref = $GV->{ADBH}->selectall_arrayref($sql);

  for (my $i=0;$i<scalar(@$UCAref);$i++){
    my ($uid,$geneId,$locusId,$modDate,$status,$owner,$chr,$start,$end,$dbVer,$working_group) = @{$UCAref->[$i]};
#    my $UCAtablerow = "<td>$owner</td><td>$modDate</td><td>(&{$GV->{GenomeContextLinkFunction}}($chr,$start,$end,$dbVer))</td><td>$$ownedRef{$uid}";
	...
	$recentClass="old_anno $status";
      ...
      push(@{$statusHash{$status}},"<tr class= \"$recentClass\"><td>$uid</td><td><a class=\"link_text hover_pointer bold\" target=\"_blank\" onclick=\"checkout('$uid');\">$geneId</a></td><td>$locusId</td><td>$owner</td><td>$working_group</td><td>$dbVer</td><td>$modDate</td><td><a class='hidelinkicon' target='_blank' href='".encode_entities(&{$GV->{GenomeContextLinkFunction}}($chr,$start,$end,$dbVer))."'>$chr $start $end</a></td><td>$$ownedRef{$uid}</td></tr>");
    }elsif ($$ownedRef{$uid} eq $PRM->{USERid}){
      push(@{$statusHash{$status}},"<tr class= \"$recentClass\"><td>$uid</td><td><a class=\"link_text hover_pointer\" target=\"_blank\" onclick=\"checkout('$uid');\">$geneId</a></td><td>$locusId</td><td>$owner</td><td>$working_group</td><td>$dbVer</td><td>$modDate</td><td><a target='_blank' href='".encode_entities(&{$GV->{GenomeContextLinkFunction}}($chr,$start,$end,$dbVer))."'>$chr $start $end</a></td><td>$$ownedRef{$uid}<br /><a href='${rootPATH}${CGIPATH}AdminAnnotationGroup.pl?uid=$uid&amp;return=1'>Return Annotation</a></td></tr>");
    }else{
      push(@{$statusHash{$status}},"<tr class='$recentClass checked'><td>$uid</td><td>$geneId</td><td>$locusId</td><td>$owner</td><td>$working_group</td><td>$dbVer</td><td>$modDate</td><td><a class=\"link_text hover_pointer\" target='_blank' href='".encode_entities(&{$GV->{GenomeContextLinkFunction}}($chr,$start,$end,$dbVer))."'>$chr $start $end</a></td><td>$$ownedRef{$uid}</td></tr>");
    }
  }

##Get working_groups for display
my @WorkGroup_Array;
$sql = "SELECT distinct working_group FROM user_gene_annotation WHERE status != 'SAVED' AND dbName = '$GV->{dbTitle}' AND working_group IN (SELECT private_group FROM yrgate.user_group WHERE (gdb='$GV->{dbTitle}'or gdb='ALL') and user='$PRM->{USERid}' and status='ADMIN')";
my $WorkGroupCount = $GV->{LDBH}->selectall_arrayref($sql);
  ...
  my $page = printTitle("Private Group Admininstration ($GV->{dbTitle})","yrgate_admin-group_anno","<span class='bold indent'>Contents:</span> <span class='info'>$GV->{dbTitle} annotations administered by $PRM->{USERid}: <b>$WorkGroups</b> </span><!--a class='indent' href='$GV->{CGIPATH}AdminAnnotationGroupAll.pl'>View All DB</a -->",1,1,"AdminAnnotationGroup.pl");

  my $status = '';

  my $table = "<table class='mainT admin' border='1'><thead><tr class='headRow'><th>UID</th><th>Annotation ID<br /><span class='smaller_white'>(click to check out)</span></th><th>Locus ID</th><th>Owner</th><th>Working_Group</th><th>dbVer</th><th>Last Modified</th><th>Chromosome / Location</th><th>Checked out by Admin?</th></tr></thead>\n";
  
  foreach $status (sort {return $b cmp $a} keys %statusHash){
  
  	    my $trClass = 'catRow'.$status;
	    $table.= "<tr class='catRow bold hover_pointer $trClass'><td colspan='9' id=\"$status\">$status </td></tr>\n";
    $table .= join("\n", @{$statusHash{$status}});
  }
  $table .= "</table>\n";
...
<script type='text/javascript' src='/XGDB/javascript/superfish.js'></script>
...
<form name='tFrm' class="annotation_form" method='post' action="$GV->{CGIPATH}AdminAnnotationGroup.pl">
