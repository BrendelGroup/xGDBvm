	...
	if($lineCHAR[$x] =~ "-"){
	  ...
	  if($Qintron){
	    ...
	    $Qintron = 1;
          ...
          if($Qgap){
	    if($qONT < 0){
	      @QRYgaps = (@QRYgaps,($QgapSTART),$QgapLength);
	    }else{
	      @QRYgaps = (@QRYgaps,($QgapSTART-1),$QgapLength);
	    }
	  }
	  if($Qgap_a){
	    if($qONT < 0){
	      @QRYgaps = (@QRYgaps,($Qgap_aSTART),-($Qgap_aLength));
	    }else{
	      @QRYgaps = (@QRYgaps,($Qgap_aSTART-1),-($Qgap_aLength));
	    }
	  }
	  if($Qintron){
	    if($qONT < 0){
	      @QRYgaps = (@QRYgaps,-($QintronSTART),$QintronLength);
	    }else{
	      @QRYgaps = (@QRYgaps,-($QintronSTART-1),$QintronLength);
	    }
	  }
	  $Qgap = $Qgap_a = 0; $Qintron = 0;
...
#    print "WARNING: $gseg used as gseg_gi\n";
  ...
  for($x=0;$x<=$#exon;$x++){
    $sqlstm .= "(LAST_INSERT_ID()," . ($x + 1) . ",$exon[$x][0],$exon[$x][1],$exon[$x][2],$exon[$x][3],$exon[$x][4]),";
  }
  chop($sqlstm);
  print "${sqlstm};\n";
  ...
  if(scalar(@intron)){
    $sqlstm = "INSERT INTO ${table}_introns (pgs_uid,num,gseg_start,gseg_stop,Dscore,Dsim,Ascore,Asim) VALUES ";
    for($x=0;$x<=$#intron;$x++){
