...
sub ExNeighbor {
  ...
  my ($db_table, $select_table);
  if ($GENOME_TYPE eq "CHR-Based") {
    $db_table = "chr_".$db_table;
    $select_part = "select geneId, chr,     strand, l_pos, r_pos from $db_table where chr=\'$chr\' and strand=\'$ori\' and ((l_pos<=$begin and r_pos>=$begin) or (l_pos<=$end and r_pos>=$end) or (l_pos>=$begin and r_pos<=$end))";
  }
  else {
    $db_table = "gseg_".$db_table;
    $select_part = "select geneId, gseg_gi, strand, l_pos, r_pos from $db_table a left join gseg_type b on a.gseg_gi=b.gi where gseg_gi=\'$chr\' and strand=\'$ori\' and ((l_pos<=$begin and r_pos>=$begin) or (l_pos<=$end and r_pos>=$end) or (l_pos>=$begin and r_pos<=$end))";
  }
