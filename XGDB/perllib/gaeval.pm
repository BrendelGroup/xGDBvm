...
sub STRmatch{
  ...
  my ($intSTR,$annPOS,$pgsPOS,%annINTRONS,%annExInJunct);
  ...
  for($annPOS=1;$annPOS<$#$annSTR_ref;$annPOS+=2){
    ...
    $annExInJunct{$annSTR_ref->[$annPOS]} = $annSTR_ref->[$annPOS + 1];
    $annExInJunct{$annSTR_ref->[$annPOS + 1]} = $annSTR_ref->[$annPOS];
  }
  for($pgsPOS=1;$pgsPOS<$#$pgsSTR_ref;$pgsPOS+=2){
    $intSTR = $pgsSTR_ref->[$pgsPOS] . ":" . $pgsSTR_ref->[$pgsPOS + 1];
    if($annPROPS_href){
      if(exists($annExInJunct{$pgsSTR_ref->[$pgsPOS]})){
	&add_intronPROP($annPROPS_href,"EXINTJUNCT_confirmed",$pgsSTR_ref->[$pgsPOS],$pgsINFO_hr);
	if(exists($annExInJunct{$pgsSTR_ref->[$pgsPOS + 1]})){
	  &add_intronPROP($annPROPS_href,"EXINTJUNCT_confirmed",$pgsSTR_ref->[$pgsPOS + 1],$pgsINFO_hr);
	}else{
	  &add_intronPROP($annPROPS_href,"EXINTJUNCT_alternative",$annExInJunct{$pgsSTR_ref->[$pgsPOS]},$pgsINFO_hr);
	}
      }elsif(exists($annExInJunct{$pgsSTR_ref->[$pgsPOS + 1]})){
        &add_intronPROP($annPROPS_href,"EXINTJUNCT_confirmed",$pgsSTR_ref->[$pgsPOS + 1],$pgsINFO_hr);
        &add_intronPROP($annPROPS_href,"EXINTJUNCT_alternative",$annExInJunct{$pgsSTR_ref->[$pgsPOS + 1]},$pgsINFO_hr);
      }else{
	## neither splice junction matches need to test for intron as alternative(overlap) versus additional(interior)
	my $i=0; 
	while(($annSTR_ref->[$i] < $pgsSTR_ref->[$pgsPOS])&&($i <= $#$annSTR_ref)){ $i++;}
	if($i && ($annSTR_ref->[$i] > $pgsSTR_ref->[$pgsPOS])){
	  if($annSTR_ref->[$i] > $pgsSTR_ref->[$pgsPOS + 1]){ ## intron completly contained within current feature ($i-1,$i)
	    if($i % 2){ # contained within exon feature
	      &add_intronPROP($annPROPS_href,"EXONS_additionalIntron",$intSTR,$pgsINFO_hr);
	    }else{ # contained in intron feature
	      &add_intronPROP($annPROPS_href,"EXINTJUNCT_alternative",$annSTR_ref->[$i-1],$pgsINFO_hr);
	      &add_intronPROP($annPROPS_href,"EXINTJUNCT_alternative",$annSTR_ref->[$i],$pgsINFO_hr);
	    }
	  }else{
	    while(($annSTR_ref->[$i] < $pgsSTR_ref->[$pgsPOS + 1])&&($i < $#$annSTR_ref)){
	      &add_intronPROP($annPROPS_href,"EXINTJUNCT_alternative",$annSTR_ref->[$i],$pgsINFO_hr);
	      $i++;
	    }
	    &add_intronPROP($annPROPS_href,"EXINTJUNCT_alternative",$annSTR_ref->[$i],$pgsINFO_hr) if(!($i % 2));
	  }
	}
      }
    }
