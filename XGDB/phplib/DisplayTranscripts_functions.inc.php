...
//get project names and build dropdown; assign selected 
	function gseg_dropdown($GSEG, $GDB, $table_core){
		...
		$gsegQuery="select distinct gseg_gi, count(gi) as trans_count from ".$GDB.".gseg_".$table_core."_good_pgs group by gseg_gi";
		$get_gseg = $gsegQuery;
		...
		$GsegDisplay = $gseg_data['gseg_gi'].' ('.$gseg_data['trans_count'].')';
...
$Similar_Sel = compare_similar('lt.85', $Similar);
		echo "<option $Similar_Sel value=\"lt.85\">less than .85</option>";
...
$Similar_Sel = compare_similar('gt.85', $Similar);
		echo "<option $Similar_Sel value=\"gt.85\">greater than .85</option>";
