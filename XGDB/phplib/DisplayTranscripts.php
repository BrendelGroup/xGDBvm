...
require_once('/xGDBvm/XGDB/phplib/DisplayTranscripts_functions.inc.php');
?>
<?php
$pgdbmenu = "Genomes";
$DBid = $X;
...
// First create 'track' session ID for the variable that stores which track is in use for this GDB, or else use current session variable. 
// The session ID name is defined as a variable = $s_track ; example: 'GDB001trn-track or GDB002trn-track', and its value is '$track_index', associated with SITEDEF.php
// (NOTE: DisplayProteins.php and DisplayLoci.php sessions use 'GDB001pep-track' and 'GDB001locus-track, respectively, as the session master ID name)
// The session $s_track value = '$track_index', and for DisplayTranscripts.php this session variable will contain a compound index consisting of track type+index for that type (e.g TSA-0)
    $s_track = $DBid."trn-track";  // build unique identifier (session variable)

    if(isset($_GET['track']))
    {
        $track_index=mysql_real_escape_string($_GET['track']); // get data
        
    	$_SESSION[$s_track] = $track_index; // assign data to session variable
    }
        else if (isset($_POST['track']))
    {
        $track_index=mysql_real_escape_string($_POST['track']);
    ...
    {
        $track_index="TSA-0";
        $s_track = $DBid."trn-track"; 
    	$_SESSION[$s_track] = "TSA-0"; // default to the first track (index 0)
    }

	$value_array=explode("-", $track_index); // parse index (1) and type (TSA) from e.g. TSA-1
	
	    $transcript_type=$value_array[0]; // EST, CDNA, TSA 
	    $index=intval($value_array[1]); // 1
		$table_core=${$transcript_type}[$index]['table'];
		$selected_track=${$transcript_type}[$index]['track']; // user's trackname
		$selected_color=${$transcript_type}[$index]['color'];

//Now create session and other variables that retain query results upon page reload and are unique to the track (table), 
//if more than one, and are specific to each transript type (EST, CDNA, TSA)
//This prevents session ID collisions when more than one track /type is being queried in a single browser session.

$sessID=$table_core.$DBid; // a unique ID root for session variables, to include MySQL table core (e.g. 'gene') +  GDBid ($X), e.g. 'estGDB007',

$s_limit = $sessID."limit"; // for pagination
$s_page = $sessID."page";
...
//Dropdown - allow user to choose transcript track based on SITEDEF.php. Any of three $transcript_type (EST,CDNA,TSA) and subordinate tracks ([0], [1], etc) can be chosen.

   $num="";
   $trn_dropdown="<form method=\"post\" name=\"select_track\" action=\"/XGDB/phplib/DisplayTranscripts.php?GDB=$X\">";
   $trn_dropdown.="<label class=\"normalfont\"><span class=\"highlight\">Select a different Transcript Dataset (if any):</span> </label>";
   $trn_dropdown.="<select name=\"track\" size=\"1\"  onchange=\"formSubmit('select_track')\">";
   $trn_dropdown .= "<option value=\"\">Select Transcript Dataset:</option>\n\n";
   
$types = array( 'EST', 'CDNA', 'TSA');
foreach( $types as $type )
{

  foreach (${$type} as $type_index=>$sub) // cycle through track types (EST etc) based on typeindex [0], [1] of subordinate tracks, etc, if any.
  {
   $table_select=$sub['table']; // the unique MySQL table core
   $track_select=$sub['track']; // the user's trackname 
   $color_select=$sub['color']; // choice of track color
   $display_select=$sub['display']; // display (1) or hide (0)
   $display=($display_select==0)?0:1; //display defaults to 1
   $num=$type_index; // 0 to whatever for each track type (most will be just 0)
  if($display != 0) //create a post-able value for consisting of type-num (e.g. EST-1, a unique track identifier w/in this GDB)
      {
        $trn_dropdown .= "<option style=\"color:white; background-color:$color_select\" value=\"${type}-${num}\">$track_select </option>\n\n";
      }
   }
}
   $trn_dropdown.="</select></form><br />";
...
$intronGet='introns_between';
		...
		$_SESSION[$s_link[1]] ='intron_min='.$intron_min.'&amp; intron_max='.$intron_max;
}
...
$gsegGet='scaff_from_to';
		...
		$_SESSION[$s_word[1]] = $rangeGet; //add search query word to session
		$_SESSION[$s_link[1]] ='$gsegGet=$rangeGet';
}
...
//store variables in session
	  if ($fieldWord == 'no filter'){//user wants to clear filter
	  	$searchDetail[$p] = "";
			$_SESSION[$s_passed[$p]] = '';
			$_SESSION[$s_field[$p]] = "";
			$_SESSION[$s_word[$p]] = "Enter search term/region"; // no problem
		...
		if ($fieldWord == 'no filter'){ //user wants to clear search
	  	$searchDetail[$p] = '';
			...
			$_SESSION[$s_link[$p]] = '';
			 ...
			 $_SESSION[$s_link[$p]] =$fieldWord.'='.$searchWord;
				 $searchWord_str = '\''.$searchWord.'\'';
				 $searchWord_wild = '\'%'.$searchWord.'%\'';
				 $searchDetail[$p] = " AND ((a.gseg_gi = $searchWord_str) OR (a.gi LIKE $searchWord_wild) OR (b.description LIKE  $searchWord_wild))";
...
$_SESSION[$s_link[1]] ='';
//$_SESSION[$s_searchword] = "Enter search term/region-(s_searchword)"; //problem: 
...
if (isset($_POST['passed']) && $_POST['passed'] == 2) //gseg search, write to session2
	{
		...
		if ($searchWord == 'no filter'){//user wants to clear filter
			...
			$_SESSION[$s_link[$p]] ='gseg='.$searchWord;// for "get" URL
			$table = 'a.';
			$comparator = '=';
			$searchWord = '\''.$searchWord.'\'';
...
if (isset($_POST['passed']) && $_POST['passed'] == 3) //copy number search, write to session3:
	{
		...
		$_SESSION[$s_link[$p]] ='copy_number='.$searchWord;//we want to store the lt or gt before parsing
	...
	else if(isset($_SESSION[$s_passed[3]])) //If session[3] exists use those values
	{
		...
		$fieldWord = $searchCopyNum; //nested query to find copy number -- see core query components
		$searchWord = $_SESSION[$s_word[$p]];
  		$_SESSION[$s_link[$p]] ='copy_number='.$searchWord;
		$greater='/gt_([0-9]+)/';//e.g. gt_5
		$lesser='/lt_([0-9]+)/';//e.g. lt_2
		$equal='/eq_([0-9]+)/';//e.g. eq_2
 		if(preg_match($greater, $searchWord, $match)) {
...
if (isset($_POST['passed']) && $_POST['passed'] == 4) //similarity search, write to session4:
	{
		...
		if ($searchWord == 'no filter'){//user wants to clear filter
	  	...
	  	$table = 'a.';
		$_SESSION[$s_link[$p]] ='sim='.$searchWord;//we want to store the lt or gt before parsing
	...
	else if(isset($_SESSION[$s_passed[4]])) //If session[4] exists use those values
	{
		...
		$fieldWord = 'sim';
		$searchWord = $_SESSION[$s_word[$p]];
  		$_SESSION[$s_link[$p]] ='sim='.$searchWord;
		$table = 'a.';
		$greater='/gt(\.[0-9]{2})/';//e.g. gt.75
		$lesser='/lt(\.[0-9]{2})/';//e.g. lt.75
 		if(preg_match($greater, $searchWord, $match)) {
...
if (isset($_POST['passed']) && $_POST['passed'] == 5) //coverage search, write to session5:
	{
		...
		if ($searchWord == 'no filter'){//user wants to clear filter
	  	...
	  	$table = 'a.';
		$_SESSION[$s_link[$p]] ='coverage='.$searchWord;//we want to store the lt or gt before parsing
		$greater='/gt(\.[0-9]{2})/';//e.g. gt.75
		$lesser='/lt(\.[0-9]{2})/';//e.g. lt.75
 		if(preg_match($greater, $searchWord, $match)) {
 			$comparator= '>';
	...
	else if(isset($_SESSION[$s_passed[5]])) //If session[5] exists use those values
	{
		...
		$fieldWord = 'coverage';
		$searchWord = $_SESSION[$s_word[$p]];
  		$_SESSION[$s_link[$p]] ='coverage='.$searchWord;
		$table = 'a.';
		$greater='/gt(\.[0-9]{2})/';//e.g. gt.75
		$lesser='/lt(\.[0-9]{2})/';//e.g. lt.75
 		if(preg_match($greater, $searchWord, $match)) {
...
$_SESSION[$s_query] = $searchTotal;//used in csv_DisplayTranscripts.php!! For this reason, the searchTotal query needs to be IDENTICAL to the searchQuery except for the limits.
...
//GDB 	Scaffold/BAC ID 	Splice-Aligned Transcript ID 	Start position 	End position 	Description 	Date
...
<th title=\"click to view protein record\" align=\"center\">Transcript ID</th>
...
<th title=\"Click to view GenomeThreader alignment\" align=\"center\">GSQ</th>
...
//pull data from DB to correspond with headers
	...
	$transcript_id = $data_array['gi'];
	$transcript_id_link = "<span  class=\"\"><a title=\"View record at $DBid (opens a new web browser page/tab)\" target=\"_blank\" href=\"/${DBid}/cgi-bin/findRecord.pl?dbid=0;resid=4&amp;id=$transcript_id\">$transcript_id</a></span>"; ## broken!
	...
	$copy_num_link = "<span  class=\"\"><a title=\"View all locations for this spliced alignment\" href=\"/XGDB/phplib/DisplayTranscripts.php?GDB=$DBid&amp;gi=$transcript_id\">$copy_num</a></span>";
	$coverage = $data_array['cov'];
	$similarity = $data_array['sim'];
	$gth_link = "<span  class=\"\"><a title=\"View GenomeThreader alignment for $transcript_id (opens a new web browser page/tab)\" target=\"_blank\" href=\"/${DBid}/cgi-bin/getGSQ.pl?gsegSRC=${DBid}scaffold&amp;dbid=0&amp;resid=4&amp;pgs_uid=$uid\"><img src=\"/XGDB/images/GSQParam.png\" alt=\"?\" /></a></span>";
	...
	<td align=\"center\">
	    <a target=\"_blank\" href=\"/$DBid/cgi-bin/findRecord.pl?id=$transcript_id\" title=\"View record (opens a new browser window)\">$transcript_id</a>
	</td>
...
$part_a="<a id=\"selfReference\" title=\"right click to copy this link\" href=\"/XGDB/phplib/DisplayTranscripts.php?GDB=$X";
		...
		<tr>
			<td>
				<h1 class="bottommargin1 nowrap">
					<?php echo "Aligned Transcripts: <span class=\"largerfont\" style=\"border: 3px solid $selected_color; color: white; background-color: $selected_color\"> &nbsp; $selected_track &nbsp;</span>  " ; ?> &nbsp; <img id="tracks_transcripts" title="Click for Aligned Transcripts Help" style="margin-bottom:-1px" src="/XGDB/images/help-icon.png" alt="?" class="xgdb-help-button" /><span class="heading"> In order of aligned location.</span>
				</h1>
<?php echo $trn_dropdown; ?>
			</td>
		</tr>
		<tr>
			<td align="left" style="padding-bottom:1.5em">
						<?php 	$n=5;//number of queries.
							...
							Filter on; retrieved <!--span>$active_filters</span--> <b>$entries</b> $records | <a href=\"/XGDB/phplib/Display_clear_filters.php?table=${table_core}&dbid=${DBid}&n=${n}&index=${track_index}&redirect=Transcripts\">Clear&nbsp;All&nbsp;Filters</a> |</span>
							";
							}else{
							echo "
				...
				<td align="right" style="padding-bottom:1.5em">
						<span class="normalfont" style="margin-right: 1em"><a href="/XGDB/phplib/csv_DisplayTranscripts.php?track=<?php echo $track_index; ?>&amp;type=<?php echo $transcript_type; ?>&amp;GDB=<?php echo $X; ?>">Download&nbsp;.csv</a></span>
				</td>
			</tr>
	</table>
		<table id="header_functions" width="950px" border="0">
			...
			<tr valign="top">
					...
					<h2>Search:<span class="heading"> Enter category, term, click "Go".</span> </h2>
						<form method="post" name="general_search" action="/XGDB/phplib/DisplayTranscripts.php?GDB=<?php echo $X; ?>">
					...
					<h2>Filters:<span class="heading"> Multiple search filters are boolean ("AND")</span> </h2>
						<form method="post" name="gseg" action="/XGDB/phplib/DisplayTranscripts.php?GDB=<?php echo $X; ?>">
						...
						<form method="post" name="copy_number" action="/XGDB/phplib/DisplayTranscripts.php?GDB=<?php echo $X; ?>">
						<table style="margin:0; width: 100%;">
							<tr align="center">
								<td width="40%" align = "right">
								<input type="hidden" name="field3" value="status" />
							   		<span class="<?php active_search(3, isset($_SESSION[$s_passed[3]])?$_SESSION[$s_passed[3]]:""); ?> bold">Copy Number:&nbsp;<img id="tracks_copynum" title="Click for explanation" src="/XGDB/images/help-icon.png" alt="?" class="xgdb-help-button" /></span>
							   		</td>
							   		<td width="60%" align = "left">
						...
						<form method="post" name="similarity" action="/XGDB/phplib/DisplayTranscripts.php?GDB=<?php echo $X; ?>">
						<table style="margin:0 ; width: 100%;">
							<tr align="center">
								<td width="40%" align = "right">
						...
						<form method="post" name="coverage" action="/XGDB/phplib/DisplayTranscripts.php?GDB=<?php echo $X; ?>">
					...
					<h2>Pages:</h2>
							<table style="margin:5px 0 12px 10px; width: 100%;">
								<tr>
									<td>
									# / Page:<br />		
										<form method="post" action="/XGDB/phplib/DisplayTranscripts.php?GDB=<?php echo $X; ?>">
