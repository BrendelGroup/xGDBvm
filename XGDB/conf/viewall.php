...
#$VM=`uname -n |cut -d "." -f 1`; # identifies this VM. No longer using this variable here.
...
######### display GSQ, GTH Remote Job Icon ID and link (Current or Locked GDB) ########
    ...
    ######## Display logfile(s) in table (Locked or Current only) ##########
    # NOTE: id and title are passed to javascript. Since ids in the DOM must each be unique, we assign id= the (unique) whole path to the logfile, and assign name = GDB ID.
    
    $logfile_display= ($Status =='Locked' || $Status =='Current' )? "<img src='' alt='' id='${XGDB_DATADIR}${DBid}/logs/Pipeline_procedure.log' name='$ID' title='Procedure log for $DBid (popup)' class='logfile-button' />":"";
    $logfile_display.= ($Status =='Locked' || $Status =='Current' )? "<img src='' alt='' id='${XGDB_DATADIR}${DBid}/logs/Pipeline_error.log' name='$ID' title='Error log for $DBid (popup)' class='logfile-button' />":"";
    $logfile_display.= ($CpGAT_Status=="Yes" && ($Status =='Locked' || $Status =='Current' ))? "<img src='' alt='' id='${XGDB_DATADIR}${DBid}/logs/CpGAT_procedure.log' name='$ID' title='CpGAT procedure log for $DBid (popup)' class='logfile-button' />":"";
...
?>
    ...
    <div id="maincontentscontainer" class="twocolumn overflow configure">
            ...
            <h1 class="configure bottommargin1">
                ...
                List All Configured
                <span class="<?php echo $showOne; ?>">: <?php echo $DBid; ?> </span> 
                <span class="bigfont <?php echo $showOne; ?>">(<a href="/XGDB/conf/viewall.php">View All GDB</a>)</span> 
                <img id='config_viewall' title='Here you view or manage configured GDB. Click ? for details.' class='help-button nudge1' src='/XGDB/images/help-icon.png' alt='?' /> <?php echo $error_message ?>
            </h1>
